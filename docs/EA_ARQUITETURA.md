...
| E5.6 | 2 EAs em demo com ordens reais | **pendente -- exige pregao**; fecha junto com o E4 do 123 (`config/ea_123.yaml`, EAS_DE_PRECO.md 5.4 passo 7b) |
...
| SendCancelOrders (plural) | **REABERTA na v3.31, so' na SUBIDA** (`limpeza_na_subida`): cobre o processo que MORREU com ordens vivas, que a reconciliacao ordem a ordem nao cobre. Guardada pela vaga (nao cancela ordens de outro EA real). **TESTADA AO VIVO em 23/09: passou** (retorno 0, ordem manual cancelada no
Profit). Antes da abertura e' recusada com NL_INVALID_ARGS -- a v3.38 repete
ate' conseguir. Motivo da reabertura: stop e alvo NAO sao OCO na corretora -- perna orfa abre posicao |
| Reconciliacao com posicao INVERTIDA | **CORRIGIDO (v3.31)**: se as duas pernas executarem com o EA parado, a posicao real inverte; antes caia no ramo generico (`nao_executou`, posicao aberta e sem protecao). Agora qualquer posicao real != esperada e != 0 -> cancela tudo do ativo e zera. Teste reprova no codigo antigo com `limpou_pendentes` |
...
| **123** | fundo de 3 barras a favor da MME80; stop de compra acima da 3a, stop na minima da 2a; alvo simetrico | agressao/delta na barra de rompimento (mesma porta do ORB) | **F4 real-pequena -> F5 desenhado** |
